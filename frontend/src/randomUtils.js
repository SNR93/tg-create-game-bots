 ...
 * Frontend-утилиты случайности, совместимые с backend-логикой.
 * Нужны для симулятора и отображения поведения нод без обращения к Telegram.
 * Код должен оставаться синхронным и детерминированным в пределах входных настроек.
...
export function normalizeRandomConfig(data = {}) {
...
export function randomConfigErrors(data = {}) {
  const config = normalizeRandomConfig(data);
  if (config.legacy) return [];
  const errors = [];

  if (!Number.isInteger(Number(data.rangeMin)) || !Number.isInteger(Number(data.rangeMax))) errors.push('Границы случайного числа должны быть целыми числами.');
  ...
  for (const branch of config.branches) {
    ...
    if (branch.from < config.rangeMin || branch.to > config.rangeMax) errors.push(`Вариант «${branch.label || '?'}» выходит за общий диапазон ${config.rangeMin}..${config.rangeMax}.`);
  }
  for (let index = 1; index < sorted.length; index++) {
    if (sorted[index].from <= sorted[index - 1].to) errors.push(`Диапазоны вариантов «${sorted[index - 1].label || '?'}» и «${sorted[index].label || '?'}» пересекаются.`);
  }
...
export function pickRandomBranch(data = {}, random = Math.random) {
  const config = normalizeRandomConfig(data);
